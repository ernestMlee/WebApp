...
function ajax_getBestResult(url, parameters)
{
    $.ajax({
        type: "POST",
        url: url, 
        ...
        success: function(data) { 
			...
			var IndividualTimer = TimerList[2].split(":");
			//append BEST GAME TIME to SCOREBOARD
			$("#Game_BestTimer_Minute").html(IndividualTimer[0]);
			$("#Game_BestTimer_Second").html(IndividualTimer[1]);
		},
		complete: function(data) { },
        error: function(request, status,error) { }
	});
...
function ajax_getLeaderboard(url, parameters)
{
    $.ajax({
        type: "POST",
        url: url, 
        ...
        success: function(data) { 
			...
			for(i = 0; i < playerList.length; i++)
			{
				...
				if(i == 0)
				{
					$("#Rugby_Scoreboard").append("<tr><td class= 'whiteleft'>" + name + "</td><td class= 'whitecenter' >" + minute +"&nbsp;<span class='LeaderTimerText'>min</span></td><td class= 'whiteright'>" + second + "&nbsp;<span class='LeaderTimerText'>sec</span></td></tr>");
				}
				else
				{
					$("#Rugby_Scoreboard").append("<tr><td class= 'orangeleft'>" + name + "</td><td class= 'orangecenter'>" + minute + "&nbsp;<span class='LeaderTimerText'>min</span></td><td class= 'orangeright'>" + second + "&nbsp;<span class='LeaderTimerText'>sec</span></td></tr>");
				}
			}
			
			for(i = 0; i < (10 - playerList.length); i++)
			{
				$("#Rugby_Scoreboard").append("<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>");
			}
		},
		complete: function(data) { },
        error: function(request, status,error) { }

...
(function($){
	...
	$.fn.touchgallery = function(options){
		return this.each(function(){
			...
			/*
			var $navbuttons = $('<div style="position:absolute; width: 125; height: 120; left: 40; top: 400; text-align: left; line-height:' + setting.height + 'px; cursor: none; opacity: 200" />')
				.clone()
				.addBack()
				.eq(0)
				.data({'dir': 'back'})
				...
				.eq(1)
				.data({'dir': 'forth'})
				.css({left: 'auto', right: 40, textAlign: 'right'}).html('<img src="' + setting.navbuttons[1] + '" style="vertical-align: middle; position:relative; top:' + setting.navbuttons[2] +'px" title="Forward one image" />')
				.end()
			...
			*/
			...
			function navigate(keyword){
				...
				catalogue_index = setting.curslide;
				toggle_price();
			}
